/**
*@Brief Code to perform various array computations,
* These consist of printing the elements of an array and
* computing the: dot products between vectors,
* length of a vector, L^1 norm of a vector, vector max norm,
* cross product, unit vector and the angle between two vectors.
...
{
    double euclid_length = sqrt(dotprod(v,v,n));  /*Calling dotprod to compute the dot product between v and itself*/
...
double angle_between_vecs(const double u[], const double v[], const int n)
{
    ...
    double theta = acos( dotprod(u,v,n) /( length(u,n) * length(v,n) ) );
